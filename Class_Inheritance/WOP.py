...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        prompt_answer = input("Guess a letter, phrase, or type 'exit' or 'pass':")
        while notValidAnswer:
            prompt_answer = input("Guess a letter, phrase, or type 'exit' or 'pass':")
            if len(prompt_answer) == 1 and prompt_answer.isalpha():
                notValidAnswer = False
                return prompt_answer
            elif len(prompt_answer) > 1 and prompt_answer not in ['exit', 'pass']:
                notValidAnswer = False
                return prompt_answer
            elif prompt_answer == 'exit':
                notValidAnswer = False
                return prompt_answer
            elif prompt_answer == 'pass':
                notValidAnswer = False
                return prompt_answer
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        choice_value = random.randint(1,10)
        if choice_value > self.difficulty:
            return False
        else:
            return True
    ...
    def getPossibleLetters(self, guessed):
        ...
        if self.prizeMoney < VOWEL_COST:
            for letter in LETTERS:
                if letter in VOWELS:
                    continue
                elif letter in guessed.upper():
                    continue
                else:
                    lettersReturned.append(letter)
        else:
            for letter in LETTERS:
                if letter not in guessed.upper():
                    lettersReturned.append(letter)
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if flip:
            most_freq_char = self.SORTED_FREQUENCIES.find(possibleLetters[0])
            for letter in possibleLetters:
                if self.SORTED_FREQUENCIES.find(letter.upper()) > most_freq_char:
                    most_freq_char = self.SORTED_FREQUENCIES.find(letter.upper())
            return SORTED_FREQUENCIES[most_freq_char]
        else:
